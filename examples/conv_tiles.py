...
def _offline_mem_create(size, flags=0):
    ...
    dma_addr = _offline_dma_cursor
    _offline_dma_cursor += ((size + 0xfff) // 0x1000) * 0x1000
    return rknpu_mem_create(
        handle=0,
        flags=flags,
        size=size,
        obj_addr=dma_addr,
...
def mem_allocate(fd, size, flags=0):
    if NO_DEVICE:
        return bytearray(size), _offline_mem_create(size, flags)
...
def mem_sync(mem_create, flags, size=None, offset=0):
    ...
    sync = rknpu_mem_sync(
        ...
        offset=offset,
        size=mem_create.size if size is None else size,
    )
...
def npu_reset(fd):
    if NO_DEVICE:
        return 0
    return npu_action(RKNPU_ACT_RESET, 0)
...
def npu_submit(task_count=1, flags=RKNPU_JOB_PC, task_start=0, core_mask=1, subcore_tasks=None):
    ...
    submit_struct = rknpu_submit(
        ...
        core_mask=core_mask, fence_fd=-1,
    )
    if subcore_tasks is None:
        subcore_tasks = (
            (task_start, task_count),
            (task_start + task_count, 0),
            (task_start + task_count, 0),
        )
...
def make_rknn_full_conv2d_regs_from_desc(p, desc, in_dma, wt_dma, out_dma):
    ...
    compiler_data_channels = min(p["in_c"], 32)
    if p["kh"] == 1 and p["kw"] == 1 and (
        (p["in_c"] == 256 and p["out_c"] == 512 and p["in_h"] == 14) or
        (p["in_c"] == 64 and p["out_c"] == 128 and p["in_h"] == 56)
    ):
        compiler_data_channels = 64
    compiler_data_size1 = ((compiler_data_channels - 1) << 16) | _align_up(p["in_c"], p["align_c"])
    ...
    compiler_surface_add = reg_value(reg.SURFACE_ADD)
    if p["kh"] == 1 and p["kw"] == 1 and (
        (p["in_c"] == 256 and p["out_c"] == 512 and p["in_h"] == 14) or
        (p["in_c"] == 64 and p["out_c"] == 128 and p["in_h"] == 56)
    ):
        compiler_cvt_con0 = 0x0b
    if p["kh"] == 1 and p["kw"] == 1 and (
        (p["in_c"] == 256 and p["out_c"] == 512 and p["in_h"] == 14) or
        (p["in_c"] == 64 and p["out_c"] == 128 and p["in_h"] == 56)
    ):
        compiler_surface_add = p["out_width_stride"] * 32
    ...
    return [
        ...
        *dcomp_amounts,
        ...
        E(reg.DPU, 0x40a8, 0),
        E(reg.DPU, 0x40ac, 0),
        E(reg.DPU, reg.SURFACE_ADD, compiler_surface_add),
...
def plan_observed_spatial_tile_descs(p, in_c, out_c, kh, kw, in_h, in_w, groups, stride):
    # This is a catalogue of RKNN-observed tiling templates. It computes DMA
    # offsets and decoded register bodies from shape fields, but it must not be
    # treated as a full RKNN tiler until the remaining template tables are
    ...
    observed_cbuf0 = 0x39 if in_h >= 32 else {7: 0xb1, 14: 0xa2, 16: 0x93, 20: 0x84, 24: 0x66, 28: 0x48}.get(in_h, None)
    families = [
        ("setup", [(0, out_c)]),
...
def direct_spatial_default_supported(descs):
    return descs is not None and (
        direct_spatial_pc_root6_supported(descs) or
...
def direct_spatial_sixdesc_supported(descs):
    return (
        direct_spatial_sixdesc_h7_supported(descs) or
        direct_spatial_sixdesc_h14_supported(descs) or
        direct_spatial_sixdesc_c32_h14_supported(descs) or
        direct_spatial_sixdesc_pw_c256_h14_supported(descs)
    )
