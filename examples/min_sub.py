...
npu_regs = [
    ...
    (0x1001 << 48) | (0x108402c0 << 16) | 0x4070,  # REG_DPU_EW_CFG: ew_alu_algo=4 (SUB), ew_op_type=0, is_cvt_bypass=0
...
for i in range(8):
    inputs[i] = struct.unpack('<H', struct.pack('<e', 8.0))[0]
    weights[i] = struct.unpack('<H', struct.pack('<e', 3.0))[0]
