...
class ConvLowering:
    params: dict
    is_spatial: bool
    grouped_spatial: bool
    split_add_chunks: list | None
    hw_out_fp16: bool
...
@dataclass(frozen=True)
class ConvChannelSlice:
    oc_tile: int
    weight_offset: int
    surface_offset: int

def _is_pointwise_reduce(in_c, out_c, kh, kw, groups):
    return groups == 1 and kh == 1 and kw == 1 and in_c >= 96 and out_c <= 128
...
def _pointwise_split_add_chunks(in_c, out_c, in_h, in_w, kh, kw, groups):
    ...
    if (in_h < 4 or in_w < 4) and 64 <= in_c <= 256:
        return [(0, in_c)]
    if _is_pointwise_reduce(in_c, out_c, kh, kw, groups):
        chunk_c = 128 if out_c > 32 else 64
        return [(start, min(start + chunk_c, in_c)) for start in range(0, in_c, chunk_c)]
    return None

def _lower_conv2d_params(batch, in_c, in_h, in_w, out_c, kh, kw, groups, out_fp16):
    ...
    hw_out_fp16 = (out_fp16 or is_spatial or p["is_depthwise"] or out_c >= 128
                   ...
                   or split_add_chunks is not None)
    return ConvLowering(p, is_spatial, grouped_spatial, split_add_chunks, hw_out_fp16)
...
def _conv_task_slices_like_mesa(p, is_spatial, grouped_spatial, logical_out_c):
    pc_chain_tiles, raw_tiles = _conv_tiles(p, is_spatial, grouped_spatial)
    slices = []
    for row_start, tile_in_h in raw_tiles:
        tile_p = p if tile_in_h == p["in_h"] else _conv_params(
            1, p["in_c"], tile_in_h, p["in_w"], logical_out_c,
            p["kh"], p["kw"], p["groups"])
        slices.append(ConvTaskSlice(
            row_start=row_start,
            tile_in_h=tile_in_h,
            tile_out_h=tile_p["out_h"],
            output_offset=row_start * p["out_w"] * 16,
            params=tile_p))
    return pc_chain_tiles, slices

def _output_channel_slices_like_mesa(p, in_c, kh, kw, groups, hw_out_c, read_bytes, tile_size):
    if kh == 1 and kw == 1 and groups == 1 and in_c >= 64:
        aligned_in_c = max(32, _align_up(in_c, 32))
    else:
        aligned_in_c = _align_up(in_c, p["align_c"])
    for oc_start in range(0, hw_out_c, tile_size):
        oc_tile = min(tile_size, hw_out_c - oc_start)
        yield ConvChannelSlice(
            oc_tile=oc_tile,
            weight_offset=oc_start * kh * kw * aligned_in_c * FP16_BYTES,
            surface_offset=(oc_start // 16) * p["out_width_stride"] * 16 * read_bytes)

def _prepare_conv_weights_like_mesa(weight_nchw, in_c, out_c, kh, kw, groups, p, grouped_spatial):
    ...
    return pack_conv_weights_for_shape(weight_full, out_c, in_c, kh, kw, p["align_c"], groups)
...
def submit_pointwise_to_output(input_nchw, weight_nchw, out_c):
    ...
    input_offset = 0
    _, slices = _conv_task_slices_like_mesa(p, False, False, out_c)
    for task_slice in slices:
        input_tile = input_nchw[0, :, task_slice.row_start:task_slice.row_start + task_slice.tile_in_h, :]
        tile_p = task_slice.params
        ...
        output_offset = task_slice.output_offset
        if sched_out_c >= 48:
            hw_out_c = _align_up(sched_out_c, 16)
            for channel_slice in _output_channel_slices_like_mesa(p, in_c, 1, 1, 1, hw_out_c, FP16_BYTES, 16):
                regs = make_conv2d_regs(
                    1, in_c, task_slice.tile_in_h, in_w, channel_slice.oc_tile, 1, 1,
                    input_mem_create.dma_addr + input_offset,
                    weight_mem_create.dma_addr + channel_slice.weight_offset,
                    output_mem_create.dma_addr + output_offset + channel_slice.surface_offset,
                    groups=1,
                    out_width_stride_override=p["out_width_stride"],
                    full_data_bank=True,
                    out_fp16=True)
                regs = _with_cbuf_data_bank(regs, _tile_data_bank(p, task_slice.tile_in_h))
                task_regs.append(regs)
        else:
            regs = make_conv2d_regs(
                1, in_c, task_slice.tile_in_h, in_w, sched_out_c, 1, 1,
                input_mem_create.dma_addr + input_offset,
...
def _submit_depthwise_channel_slice_like_mesa(task_slice, input_tile, weight_nchw, result,
                                             sample_idx, out_c, kh, kw, in_w, out_w,
                                             p, hw_out_fp16, unpack_c2, read_output, wt_ptr):
    ...
    for ch_start in range(0, out_c, depthwise_block_c):
        ...
        block_p = _conv_params(1, hw_ch_tile, task_slice.tile_in_h, in_w, hw_ch_tile, kh, kw, hw_ch_tile)
        block_input = np.zeros((hw_ch_tile, task_slice.tile_in_h, in_w), dtype=np.float16)
        block_input[:ch_tile] = input_tile[ch_start:ch_start + ch_tile]
        block_input_flat = _pack_conv_input_fp16(block_input, block_p).view(np.uint16).tolist()
        ...
        ctypes.memset(ctypes.addressof(ctypes.c_char.from_buffer(output_map)), 0, output_mem_create.size)
        regs = make_conv2d_regs(
            1, hw_ch_tile, task_slice.tile_in_h, in_w, hw_ch_tile, kh, kw,
        ...
        result[sample_idx, ch_start:ch_start + ch_tile, task_slice.row_start:task_slice.row_start + task_slice.tile_out_h, :] = _unpack_flat_1x1_output(
            out_buf, hw_ch_tile, task_slice.tile_out_h, out_w, p["out_width_stride"], unpack_c2)[:ch_tile]
...
def run_conv2d(batch, in_c, out_c, kh, kw, input_hw, groups=1, weight_in_c=None, out_fp16=False):
    ...
    lowering = _lower_conv2d_params(1, in_c, in_h, in_w, hw_sched_out_c, kh, kw, groups, out_fp16)
    p = lowering.params
    is_spatial = lowering.is_spatial
    out_h, out_w = p["out_h"], p["out_w"]
    grouped_spatial = lowering.grouped_spatial
    hw_out_fp16 = lowering.hw_out_fp16
    ...
    split_add_chunks = lowering.split_add_chunks
    ...
    wt_flat = _prepare_conv_weights_like_mesa(weight_nchw, in_c, logical_out_c, kh, kw, groups, p, grouped_spatial)
    wt_ptr = ctypes.addressof(ctypes.c_char.from_buffer(weight_map))
    ctypes.memmove(wt_ptr, wt_flat.ctypes.data, wt_flat.nbytes)
    ...
    for n in range(batch):
        pc_chain_tiles, task_slices = _conv_task_slices_like_mesa(p, is_spatial, grouped_spatial, out_c)
        ...
        for task_slice in task_slices:
            tile_p = task_slice.params
            tile_out_h = task_slice.tile_out_h
            input_tile = input_nchw[n, :, task_slice.row_start:task_slice.row_start + task_slice.tile_in_h, :]
            input_flat = _pack_conv_input_fp16(input_tile, tile_p).view(np.uint16)
            ...
            if pc_chain_tiles:
                ...
                if depthwise_channel_tiles:
                    _submit_depthwise_channel_slice_like_mesa(
                        task_slice, input_tile, weight_nchw, result, n, out_c,
                        kh, kw, in_w, out_w, p, hw_out_fp16, unpack_c2,
                        read_output, wt_ptr)
                elif output_channel_tiles:
                    for channel_slice in _output_channel_slices_like_mesa(p, in_c, kh, kw, groups, hw_sched_out_c, read_bytes, output_channel_tile_size):
                        regs = make_conv2d_regs(
                            1, in_c, task_slice.tile_in_h, in_w, channel_slice.oc_tile, kh, kw,
                            input_mem_create.dma_addr + input_offset,
                            weight_mem_create.dma_addr + channel_slice.weight_offset,
                            output_mem_create.dma_addr + output_offset + channel_slice.surface_offset,
                            groups=groups,
                        ...
                        if compact_tail:
                            ...
                            regs = _with_reg_value(regs, reg.CNA, reg.CNA_DMA_CON2, (tile_p["width_stride"] * (task_slice.tile_in_h - 4)) & 0x0fffffff)
                    ...
                    task_regs.append(make_conv2d_regs(
                        1, in_c, task_slice.tile_in_h, in_w, out_c, kh, kw,
                        input_mem_create.dma_addr + input_offset,
                        weight_mem_create.dma_addr,
                        output_mem_create.dma_addr + output_offset,
                        groups=groups,
            ...
            submit_conv_tasks(task_regs, repeat=_direct_submit_repeat(p, batch, is_spatial, len(task_slices)))
            ...
            if not is_spatial:
                ...
                result[n, :, task_slice.row_start:task_slice.row_start + tile_out_h, :] = _unpack_flat_1x1_output(
