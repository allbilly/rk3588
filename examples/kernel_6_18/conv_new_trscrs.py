...
def translateCRSToFeatureData(input_crs, channelsPerGroup=0, width_stride=None,
                              pad_value=np.float16(0), output_size=None):
    """
    Pack CRS FP16 data into CDMA_DC feature format: C1 x H x W x C2.
    This is NVDLA translateCRSToFeatureData with RK width-stride padding.
    """
    if input_crs.ndim != 3:
        raise ValueError("translateCRSToFeatureData expects CRS input with shape (C, H, W)")

    channels, height, width = input_crs.shape
    cf = channelsPerGroup if channelsPerGroup != 0 else FP16_ATOM_ELEMENTS
    cfg = channels // cf
    cp = channels % cf
    cpg = 1 if cp else 0
    groups = cfg + cpg
    ...
    ff_size = groups * height * width_stride * cf
    ...
    src = input_crs.astype(np.float16, copy=False)
    dest = np.full(out_size, pad_value, dtype=np.float16)
    n = 0
    for ind_cfg in range(groups):
        for ind_r in range(height):
            for ind_s in range(width_stride):
                for ind_c in range(ind_cfg * cf, ind_cfg * cf + cf):
                    if ind_s < width and ind_c < channels:
                        dest[n] = src[ind_c, ind_r, ind_s]
                    else:
                        dest[n] = pad_value
                    n += 1

    if n != ff_size:
        raise RuntimeError(f"translateCRSToFeatureData packed {n} values, expected {ff_size}")
    return dest
...
def translate_crs_fp16_mesa(input_nchw, p):
    ...
    return translateCRSToFeatureData(
        input_nchw, channelsPerGroup=c2, width_stride=p["width_stride"],
        output_size=c1 * in_h * p["width_stride"] * c2)

def _pack_cdma_dc_feature_input_fp16(input_nchw, p):
    return translate_crs_fp16_mesa(input_nchw, p)
            ...
            for out_row_start in range(0, out_h, row_tile_h):
                ...
                for oc_start in range(0, out_c, oc_tile):
                        ...
                        for ic in range(in_c):
                            for ky in range(kh):
                                for kx in range(kw):
                                    ...
                                    flat += 1
                    wt_flat = pack_conv_weights_for_shape(tile_weight, hw_out_c, flat_c, 1, 1, tile_p["align_c"], 1)
                    ctypes.memmove(wt_ptr, wt_flat.ctypes.data, wt_flat.nbytes)
                    ctypes.memset(ctypes.addressof(ctypes.c_char.from_buffer(output_map)), 0, output_mem_create.size)

                    task_regs = [make_conv2d_regs(
                        1, flat_c, hw_tile_h, out_w, hw_out_c, 1, 1,
                    ...
                    result[n, oc_start:oc_end, out_row_start:out_row_start + tile_out_h] = _unpack_flat_1x1_output(
                        out_buf, hw_out_c, hw_tile_h, out_w, tile_p["out_width_stride"], UNPACK_C2)[:tile_out_c, :tile_out_h]
            ...
            for out_row_start in range(0, out_h, row_tile_h):
                ...
                for ch_start in range(0, out_c, channel_tile):
                    ...
                    result[n, ch_start:ch_end, out_row_start:out_row_start + tile_out_h] = _unpack_nc1hwc2_output(
                        out_buf, tile_c, hw_tile_out_h, out_w, UNPACK_C2, tile_p["out_width_stride"])[:, :tile_out_h]
        return result, input_nchw, weight_nchw
        ...
        for n in range(batch):
            for out_row_start in range(0, out_h, row_tile_h):
                ...
                for oc_start in range(0, out_c, oc_tile):
                    ...
                    wt_flat = pack_conv_weights_for_shape(tile_weight, hw_out_c, in_c, kh, kw, tile_p["align_c"], groups)
                    ctypes.memmove(wt_ptr, wt_flat.ctypes.data, wt_flat.nbytes)
                    ctypes.memset(ctypes.addressof(ctypes.c_char.from_buffer(output_map)), 0, output_mem_create.size)

                    task_regs = [make_conv2d_regs(
                        1, in_c, hw_tile_h, in_w, hw_out_c, kh, kw,
                        input_mem_create.dma_addr,
                        weight_mem_create.dma_addr,
                        output_mem_create.dma_addr,
                        groups=groups,
