...
def _pointwise_weight_atom_groups(in_c):
    return _ceil_div(max(in_c, FP16_ATOM_ELEMENTS), WEIGHT_ATOMIC_ELEMENTS)
...
def _pointwise_oc_tile_c(in_c):
    # Mesa sizes weights with calc_weights_banks(); for this FP16 schedule keep
    # one 32KiB CBUF bank worth of pointwise weights per tile.
...
# TODO line stride and surface_stride shd be 
...
# TODO     if task.input_channels_real == 1:
...
def _conv_params(batch, in_c, in_h, in_w, out_c, kh, kw, groups, stride=1):
    ...
    align_c = WEIGHT_ATOMIC_ELEMENTS if _uses_pointwise_weight_atom_layout(in_c, kh, kw, groups) else _conv_align_c(in_c, groups, out_c)
...
def pack_weights(weight_full, out_c, in_c, kh, kw, align_c, groups, layout=None):
    layout = layout or choose_weight_layout(out_c, in_c, kh, kw, align_c, groups)
    ...
    return _pack_crs_order(weight_full, out_c, in_c, kh, kw, align_c, layout)
...
def make_conv2d_regs(batch, in_c, in_h, in_w, out_c, kh, kw, in_dma, wt_dma, out_dma, groups=1, stride=1, out_width_stride_override=None, weight_reuse=False, full_data_bank=False):
    ...
    # TODO     cvt_con5 = 65535 if task.input_channels_real == 1 else 0
    ...
    npu_regs = [
        ...
        # TODO missing comment, is this the same as     
    ...
    #     con1 |= CNA_CONV_CON1_CONV_MODE(3)
        ...
        # TODO feature_grains fucntion too comples, conv_mesa.py is just 50 + task.stride_y + 1
        ...
        # TODO, conv_mesa.py is just input_width and input_height
        ...
        E(reg.CNA, reg.CNA_DATA_SIZE1,
            # TODO better naming input_channels_real - 1 
            ((in_c - 1 << 16) |             # CNA_DATA_SIZE1_DATAIN_CHANNEL_REAL
            # better name input_channels
        ...
        E(reg.CNA, reg.CNA_WEIGHT_SIZE2,
            ((kw << 24) |                  # CNA_WEIGHT_SIZE2_WEIGHT_WIDTH
             ...
             # 1 if depthwise else _align(op.output_channels, 2)
        ...
        # missing pad_left and pad_top, check README.md
...
def _input_ptr():
    return ctypes.addressof(ctypes.c_char.from_buffer(input_map))

def _weight_ptr():
    return ctypes.addressof(ctypes.c_char.from_buffer(weight_map))

def _output_ptr():
    return ctypes.addressof(ctypes.c_char.from_buffer(output_map))

def copy_input_words(input_words, offset=0):
    words = np.ascontiguousarray(input_words.view(np.uint16))
    assert offset + words.nbytes <= input_mem_create.size, "input buffer too small"
    ctypes.memmove(_input_ptr() + offset, words.ctypes.data, words.nbytes)
    return words.nbytes

def copy_input_tile(input_tile, p, offset=0):
    return copy_input_words(_pack_cdma_dc_feature_input_fp16(input_tile, p).view(np.uint16), offset)

def copy_weight_flat(wt_flat):
    flat = np.ascontiguousarray(wt_flat)
    assert flat.nbytes <= weight_mem_create.size, "weight buffer too small"
    ctypes.memmove(_weight_ptr(), flat.ctypes.data, flat.nbytes)

def clear_output():
    ctypes.memset(_output_ptr(), 0, output_mem_create.size)

def read_output_fp16(count):
    return np.frombuffer(output_map, dtype=np.uint16, count=count).copy().view(np.float16)

def submit_task_regs(task_regs):
    write_regs_to_npu_task(task_regs)
    npu_submit(task_count=len(task_regs))

def make_conv2d_regs_from_params(p, in_dma, wt_dma, out_dma, **overrides):
    return make_conv2d_regs(
        p["batch"], p["in_c"], p["in_h"], p["in_w"], p["out_c"], p["kh"], p["kw"],
        in_dma, wt_dma, out_dma,
        groups=p["groups"],
        stride=p["stride"],
        **overrides)

def submit_one_tile(input_tile, p, wt_flat, task_regs, output_count, clear=True):
    copy_input_tile(input_tile, p)
    if wt_flat is not None:
        copy_weight_flat(wt_flat)
    if clear:
        clear_output()
    submit_task_regs(task_regs)
    return read_output_fp16(output_count)

def _output_count(p, out_c1=None, width_stride=None):
    out_c1 = out_c1 if out_c1 is not None else _ceil_div(p["align_out_c"], UNPACK_C2)
    width_stride = width_stride if width_stride is not None else p["out_width_stride"]
    return out_c1 * width_stride * UNPACK_C2

def submit_and_unpack_flat(input_tile, p, wt_flat, task_regs, out_c, out_h, out_w, hw_out_c=None, hw_out_h=None):
    hw_out_c = hw_out_c if hw_out_c is not None else out_c
    hw_out_h = hw_out_h if hw_out_h is not None else out_h
    out_buf = submit_one_tile(input_tile, p, wt_flat, task_regs, _output_count(p))
    return _unpack_flat_1x1_output(
        out_buf, hw_out_c, hw_out_h, out_w, p["out_width_stride"], UNPACK_C2)[:out_c, :out_h]

def submit_and_unpack_nc1(input_tile, p, wt_flat, task_regs, out_c, out_h, out_w, hw_out_h=None):
    hw_out_h = hw_out_h if hw_out_h is not None else out_h
    out_buf = submit_one_tile(input_tile, p, wt_flat, task_regs, _output_count(p))
    return _unpack_nc1hwc2_output(
        out_buf, out_c, hw_out_h, out_w, UNPACK_C2, p["out_width_stride"])[:, :out_h]
...
def run_conv2d(batch, in_c, out_c, kh, kw, input_hw, groups=1, weight_in_c=None, stride=1):
    ...
    spatial_im2col = is_spatial and groups == 1 and not p["is_depthwise"] and (spatial_weight_banks > RK_CBUF_BANKS // 3 or spatial_output_bytes > output_mem_create.size)
    spatial_oc_serial = is_spatial and groups == 1 and not p["is_depthwise"] and out_c > UNPACK_C2 and (in_c % 16 != 0 or in_c >= 16)
    depthwise_spatial_tiled = p["is_depthwise"] and is_spatial and (out_h > _depthwise_tile_h(out_c, out_h, in_w, kh, kw, stride) or out_c > p["align_c"])

    if not grouped_serial and not spatial_im2col and not spatial_oc_serial and not depthwise_spatial_tiled:
        ...
        if p["is_depthwise"] and in_c == 32 and out_c == 32 and not is_spatial:
            ...
            wt_flat = pack_weights(weight_full, out_c, in_c, kh, kw, p["align_c"], groups)
        copy_weight_flat(wt_flat)
        ...
        for n in range(batch):
            for out_row_start in range(0, out_h, row_tile_h):
                ...
                for oc_start in range(0, out_c, oc_tile):
                        ...
                        for ic in range(in_c):
                            for ky in range(kh):
                                for kx in range(kw):
                                    ...
                                    flat += 1
                    wt_flat = pack_weights(tile_weight, hw_out_c, flat_c, 1, 1, tile_p["align_c"], 1)

                    task_regs = [make_conv2d_regs_from_params(
                        tile_p,
                    ...
                    result[n, oc_start:oc_end, out_row_start:out_row_start + tile_out_h] = submit_and_unpack_flat(
                        im2col, tile_p, wt_flat, task_regs, tile_out_c, tile_out_h, out_w, hw_out_c, hw_tile_h)
    ...
    if grouped_serial:
        ...
        for n in range(batch):
            for g in range(groups):
                ...
                wt_flat = pack_weights(group_weight, out_per_group, input_per_group, kh, kw, gp["align_c"], 1)
                ...
                result[n, oc_start:oc_end] = submit_and_unpack_nc1(
                    input_tile, gp, wt_flat, task_regs, out_per_group, out_h, out_w)
        return result, input_nchw, weight_nchw
        ...
        for n in range(batch):
            for out_row_start in range(0, out_h, 32):
                ...
                for oc_start in range(0, out_c, oc_tile):
                    ...
                    wt_flat = pack_weights(tile_weight, tile_out_c, in_c, kh, kw, tile_p["align_c"], 1)
                    ...
                    result[n, oc_start:oc_end, out_row_start:out_row_start + tile_out_h] = submit_and_unpack_nc1(
                        input_tile, tile_p, wt_flat, task_regs, tile_out_c, tile_out_h, out_w)
        return result, input_nchw, weight_nchw

    if depthwise_spatial_tiled:
        channel_tile = min(32, out_c)
        row_tile_h = _depthwise_tile_h(out_c, out_h, in_w, kh, kw, stride)
        for n in range(batch):
            for out_row_start in range(0, out_h, row_tile_h):
                tile_out_h = min(row_tile_h, out_h - out_row_start)
                ...
                for ch_start in range(0, out_c, channel_tile):
                    ...
                    wt_flat = pack_weights(tile_weight, tile_c, tile_c, kh, kw, tile_p["align_c"], tile_c)
                    ...
                    result[n, ch_start:ch_end, out_row_start:out_row_start + tile_out_h] = submit_and_unpack_nc1(
                        input_tile, tile_p, wt_flat, task_regs, tile_c, tile_out_h, out_w, hw_tile_out_h)
        return result, input_nchw, weight_nchw
    ...
    if _needs_pointwise_oc_tile_schedule(in_c, out_c, in_h, in_w, kh, kw, groups):
        ...
        for n in range(batch):
            for out_row_start in range(0, out_h, row_tile_h):
                ...
                input_tile[:, :tile_out_h] = input_nchw[n, :, out_row_start * stride:out_row_start * stride + tile_out_h, :]
                ...
                for oc_start in range(0, out_c, oc_tile):
                    ...
                    tile_weight[:tile_out_c] = weight_nchw[oc_start:oc_end].reshape(tile_out_c, in_c, kh, kw)
                    wt_flat = pack_weights(tile_weight, hw_out_c, in_c, kh, kw, tile_p["align_c"], groups)
                    ...
                    result[n, oc_start:oc_end, out_row_start:out_row_start + tile_out_h] = submit_and_unpack_flat(
                        input_tile, tile_p, wt_flat, task_regs, tile_out_c, tile_out_h, out_w, hw_out_c, hw_tile_h)
        return result, input_nchw, weight_nchw
    ...
    for n in range(batch):
        if _needs_pointwise_tile_schedule(in_c, out_c, in_h, in_w, kh, kw, groups):
            ...
            for row_start in range(0, out_h, tile_h):
                ...
                input_bytes = copy_input_tile(input_tile, tile_p, input_offset)
                output_offset = row_start * out_w * 16
                regs = make_conv2d_regs_from_params(
                    tile_p,
            ...
            submit_task_regs(task_regs)
            out_buf = read_output_fp16(_output_count(p))
            result[n] = _unpack_flat_1x1_output(out_buf, out_c, out_h, out_w, p["out_width_stride"], UNPACK_C2)
            continue

        if not is_spatial:
            small_chan = (p["use_nhwc"] or p["mesa_aligned_small"]) and not p["is_depthwise"]
            out_c1 = _ceil_div(p["align_out_c"], UNPACK_C2)
            output_atoms_fit = output_mem_create.size // (FP16_BYTES * out_c1 * UNPACK_C2)
            ...
            tile_h = min(tile_h, max(1, output_atoms_fit // out_w)) if needs_output_tile else tile_h
...
def plan_conv_tiles(in_c, out_c, kh, kw, in_h, in_w, out_h, out_w, groups=1, stride=1):
    """
    Unified tile planner following RKNN's Y/K loop pattern.
    Returns list of tile descriptors: [{y_start, y_step, k_start, k_step, out_c_tile}]
    """
    p = _conv_params(1, in_c, in_h, in_w, out_c, kh, kw, groups, stride=stride)
    is_spatial = (kh != 1 or kw != 1)
    is_depthwise = p["is_depthwise"]
    ...
    # Bank pressure estimates
    row_bytes = width_stride * data_in_channel_aligned * FP16_BYTES
    weight_bytes_per_kernel = kh * kw * data_in_channel_aligned * FP16_BYTES
    if is_depthwise:
        weight_banks_per_tile = _ceil_div(weight_bytes_per_kernel, CBUF_BANK_SIZE)
    else:
        weight_banks_per_tile = _ceil_div(weight_bytes_per_kernel * out_c, CBUF_BANK_SIZE)

    # Y-tile step from data bank pressure  
    data_banks = max(1, _ceil_div(width_stride * data_in_channel_aligned * in_h * FP16_BYTES, CBUF_BANK_SIZE))
    y_step = out_h  # default: full height
    if data_banks > RK_CBUF_BANKS // 2:
        # Need to H-tile: reduce until data fits
        max_data_banks = RK_CBUF_BANKS // 2
        y_step = max(1, out_h * max_data_banks // data_banks)

    # K-tile step from weight bank pressure
    k_step = out_c
    weight_bank_threshold = 3  # RKNN F2 threshold
    if weight_banks_per_tile > weight_bank_threshold:
        k_step = min(MIN_CHANNEL_TILE, out_c)

    tiles = []
    for y_start in range(0, out_h, y_step):
        for k_start in range(0, out_c, k_step):
            tiles.append({
                "y_start": y_start,
                "y_step": min(y_step, out_h - y_start),
                "k_start": k_start,
                "k_step": min(k_step, out_c - k_start),
                "out_c_tile": min(k_step, out_c - k_start),
            })
    return tiles


def run_conv_gemm_style(fd, in_nchw, weight_nchw, kh, kw, stride=1, groups=1):
    """
    Clean conv executor following run_gemm pattern.
    Uses plan_conv_tiles() for unified tile planning.
    """
    batch, in_c, in_h, in_w = in_nchw.shape
    out_c = weight_nchw.shape[0]
    weight_in_c = in_c // groups
    pad = kh // 2
    out_h = (in_h + 2 * pad - kh) // stride + 1
    out_w = (in_w + 2 * pad - kw) // stride + 1

    tiles = plan_conv_tiles(in_c, out_c, kh, kw, in_h, in_w, out_h, out_w, groups, stride)
    ...
    for tile in tiles:
        y_start, y_step = tile["y_start"], tile["y_step"]
        k_start, k_step = tile["k_start"], tile["k_step"]
        tile_oc = tile["out_c_tile"]

        tile_in_h = (y_step - 1) * stride + kh if y_step < out_h else in_h
        input_slice = in_nchw[0, :, y_start * stride:y_start * stride + tile_in_h, :] if y_step < out_h else in_nchw[0]

        tile_p = _conv_params(1, in_c, tile_in_h, in_w, tile_oc, kh, kw, groups, stride=stride)

        if is_depthwise:
            tile_weight = np.zeros((tile_oc, in_c, kh, kw), dtype=np.float16)
            for i, oc in enumerate(range(k_start, k_start + tile_oc)):
                tile_weight[i, i] = weight_nchw[oc, 0]
        else:
            tile_weight = weight_nchw[k_start:k_start + tile_oc].reshape(tile_oc, weight_in_c, kh, kw)
            if groups > 1:
                tile_weight = _expand_grouped_weights(tile_weight, in_c, tile_oc, kh, kw, groups)
        if _is_grouped_spatial(in_c, tile_oc, kh, kw, groups):
            tile_weight = _reorder_grouped_spatial_weights_block16(tile_weight, tile_oc, in_c, kh, kw)

        wt_flat = pack_weights(tile_weight, tile_oc, in_c, kh, kw, tile_p["align_c"], groups)
        task_regs = [make_conv2d_regs_from_params(
            tile_p, input_mem_create.dma_addr, weight_mem_create.dma_addr,
            output_mem_create.dma_addr,
            full_data_bank=is_spatial and groups == 1)]

        result[0, k_start:k_start + tile_oc, y_start:y_start + y_step] = \
            submit_and_unpack_nc1(input_slice, tile_p, wt_flat, task_regs,
                                  tile_oc, y_step, out_w)
    return result
