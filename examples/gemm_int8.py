...
def make_gemm_regs(m, n, k, in_dma, wt_dma, out_dma):
    ...
    input_row_bytes = align_in * INT8_BYTES
    feature_grains = m + 1
    data_banks = np.clip(_ceil_div(m * input_row_bytes, CBUF_BANK_SIZE), 1, RK_CBUF_BANKS - 1)
    line_stride = max(1, _align_up(k, MIN_CHANNEL_TILE) // 16)
    ...
    npu_regs = [
        ...
        E(reg.CNA,  reg.CNA_CBUF_CON1, _ceil_div(align_in, INT8_CBUF_ENTRY_VALUES)),
        ...
        E(reg.CORE, reg.CORE_MISC_CFG, 1),
...
def run_gemm(m, n, k, a_matrix, b_matrix):
    ...
    a_matrix = np.asarray(a_matrix, dtype=np.int8).reshape(m, k)
    b_matrix = np.asarray(b_matrix, dtype=np.int8).reshape(k, n)
    input_packed = np.zeros(align_in * m, dtype=np.int8)
    input_packed.reshape(m, align_in)[:, :k] = a_matrix[:, :k]
    input_packed = input_packed.tolist()
    
    # pack_weight_tile_16x32 for int8 uses two 16-row groups per 32-output tile.
    weight = np.zeros((align_out, align_in), dtype=np.int8)
    weight_packed = np.zeros(align_out * align_in, dtype=np.int8)
    weight[:n, :k] = b_matrix.T[:n, :k]
    weight_packed[:] = weight.reshape(align_out // 32, 2, 16, align_in // 32, 32).transpose(0, 3, 1, 2, 4).ravel()
    weight_packed = weight_packed.tolist()
    
    # write np array to C array
    ct_inputs = (ctypes.c_int8 * len(input_packed)).from_buffer(input_map)
    ct_weights = (ctypes.c_int8 * len(weight_packed)).from_buffer(weight_map)
