...
C128_H2_OC256_EXACT11_SHAPE = "b1_c128_h2_w2_oc256_wic128_k1x1_g1_s1_pvalid"
C256_H2_OC64_EXACT11_SHAPE = "b1_c256_h2_w2_oc64_wic256_k1x1_g1_s1_pvalid"
C480_H14_OC16_EXACT11_SHAPE = "b1_c480_h14_w14_oc16_wic480_k1x1_g1"
C128_H1_OC24_3SETUP_SHAPE = "b1_c128_h1_w1_oc24_wic128_k1x1_g1_s1_pvalid"
...
CBUF0_OVERRIDES = {
    ...
    "b1_c256_h10_w10_oc512_wic256_k3x3_g1_s1_pvalid": 0xa2,
    "b1_c192_h7_w7_oc384_wic192_k3x3_g1_s1_pvalid": 0xb1,
    ...
    # c480_h14_oc16: weight_bank=6, data_bank=6 (per live c480_h14_oc16 GEM2)
    C480_H14_OC16_EXACT11_SHAPE: 0x066,
}
DATA_SIZE1_OVERRIDES = {
    ...
    "b1_c256_h10_w10_oc512_wic256_k3x3_g1_s1_pvalid":  0x3f0100,
    "b1_c192_h7_w7_oc384_wic192_k3x3_g1_s1_pvalid":  0x3f00c0,
    ...
    # c480_h14_oc16: in_c_real=31, in_c=480 (per live c480_h14_oc16 GEM2)
    C480_H14_OC16_EXACT11_SHAPE: 0x001f01e0,
    # c128_h1_oc24: in_c_real=63, in_c=128 (per live c128_h1_oc24 GEM2)
    C128_H1_OC24_3SETUP_SHAPE: 0x003f0080,
}
CBUF1_OVERRIDES = {
    ...
    # c128_h1_oc24: DATA_ENTRIES=4 (per live c128_h1_oc24 GEM2)
    C128_H1_OC24_3SETUP_SHAPE: 0x004,
}
...
CVT_CON0_OVERRIDES = {
    ...
    "b1_c256_h10_w10_oc512_wic256_k3x3_g1_s1_pvalid": 0x000b,
    "b1_c192_h7_w7_oc384_wic192_k3x3_g1_s1_pvalid": 0x000b,
    ...
    C128_H1_OC24_3SETUP_SHAPE: 0x000b,
}
...
DMA_CON2_OVERRIDES = {
    ...
    "b1_c256_h10_w10_oc512_wic256_k3x3_g1_s1_pvalid": 0x3c,
    "b1_c192_h7_w7_oc384_wic192_k3x3_g1_s1_pvalid": 0x15,
    ...
    C128_H1_OC24_3SETUP_SHAPE: 0x0ffffffd,
}
...
KT_TILE_SPLITS = {
    ...
    "b1_c128_h2_w2_oc256_wic128_k1x1_g1_s1_pvalid":  ((0, 96), (96, 80), (176, 80)),
    ...
    "b1_c128_h2_w2_oc256_wic128_k1x1_g1_s1_pvalid":  ((0, 96), (96, 80), (176, 80)),
    # c480_h14_oc16: 5 k_tiles summing to 16 (4+3+3+3+3), no k_half
    C480_H14_OC16_EXACT11_SHAPE: ((0, 4), (4, 3), (7, 3), (10, 3), (13, 3)),
...
CONV2_LOW_OVERRIDES = {
    ...
    "b1_c256_h10_w10_oc512_wic256_k3x3_g1_s1_pvalid": 0xd0,
    "b1_c192_h7_w7_oc384_wic192_k3x3_g1_s1_pvalid": 0xa0,
...
def _exact11_task_regs(s, in_dma, wt_dma, out_dma):
    ...
    if s["name"] == C256_H2_OC64_EXACT11_SHAPE:
        ...
        return rows
    if s["name"] == C128_H2_OC256_EXACT11_SHAPE:
        rows = _c128_h2_oc256_exact11_task_regs(s, in_dma, wt_dma, out_dma)
        if tuple(len(row) for row in rows) != layout["amounts"]:
            raise RuntimeError("c128_h2_oc256 exact11 BY_K row amounts changed")
        return rows
    if s["name"] == C480_H14_OC16_EXACT11_SHAPE:
        # 11-task no-khalf closure: 1 setup + 5 k_tiles + 5 aux (matches EXACT11_BYK_AMOUNTS)
        # All tasks write weight_size0=full (uniform cbuf, dump shows uniform value across all
        # 6 setup+5k_tile rows). Body field constants per live GEM2 capture.
        cbuf0 = 0x066
        data_size1 = 0x001f01e0
        dma2 = 0x008c
        full_weight = s["out_c"] * s["in_c"] * FP16_BYTES
        def row_setup():
            regs = _exact11_body_regs(s, "setup", 0, s["out_c"], in_dma, wt_dma, out_dma)
            return patch_regs(regs, {
                (reg.CNA, reg.CNA_DATA_SIZE1): data_size1,
                (reg.CNA, reg.CNA_CBUF_CON0): cbuf0,
                (reg.CNA, reg.CNA_CVT_CON0): 0x000b,
                (reg.CNA, reg.CNA_DMA_CON2): dma2,
                (reg.CNA, reg.CNA_WEIGHT_SIZE0): full_weight,
                (reg.CNA, reg.CNA_WEIGHT_SIZE1): full_weight >> 8,
        ...
        def row_ktile(oc_start, oc_count):
            regs = _exact11_body_regs(s, "k_tile", oc_start, oc_count, in_dma, wt_dma, out_dma)
            return patch_regs(regs, {
                (reg.CNA, reg.CNA_DATA_SIZE1): data_size1,
                (reg.CNA, reg.CNA_CBUF_CON0): cbuf0,
                (reg.CNA, reg.CNA_CVT_CON0): 0x000b,
        ...
        for oc_start, oc_count in splits:
            rows.append(row_ktile(oc_start, oc_count))
            if oc_start != last_oc_start:
                rows.append(_exact11_aux_regs(s, out_dma))
        rows.append(_exact11_aux_regs(s, out_dma))
        if tuple(len(row) for row in rows) != layout["amounts"]:
            raise RuntimeError(f"{s['name']} exact11 BY_K row amounts changed")
        return rows
    if s["name"] == C480_H14_OC16_EXACT11_SHAPE:
        # 11-task no-khalf closure: 1 setup + 5 k_tiles + 5 aux (matches EXACT11_BYK_AMOUNTS)
        # All tasks write weight_size0=full (uniform cbuf, dump shows uniform value across all
        ...
        if tuple(len(row) for row in rows) != layout["amounts"]:
            raise RuntimeError(f"{s['name']} exact11 BY_K row amounts changed")
        return rows
    if s["name"] == C128_H1_OC24_3SETUP_SHAPE:
        # 3-setup chain: 3 IDENTICAL setup rows of 108q each, all with the SAME body fields
        # (shared cbuf, dump shows only 1 set of body field writes for all 3 tasks).
        # The rknn runtime submits this exact 3-task pattern (max_error=0.0086 when run).
        # Body fields from prefix_c128_h1_oc24_s1pvalid_keep1_gem2:
        # CBUF0=0x0b1, DATA_SIZE1=0x3f0080, CBUF1=0x04, CVT_CON0=0x0b, DMA_CON2=0x0ffffffd.
        full_weight = s["out_c"] * s["in_c"] * FP16_BYTES
        def row_setup():
            regs = _exact11_body_regs(s, "setup", 0, s["out_c"], in_dma, wt_dma, out_dma)
            return patch_regs(regs, {
                (reg.CNA, reg.CNA_DATA_SIZE1): 0x003f0080,
                (reg.CNA, reg.CNA_CBUF_CON0): 0x0b1,
                (reg.CNA, reg.CNA_CBUF_CON1): 0x004,
                (reg.CNA, reg.CNA_CVT_CON0): 0x000b,
                (reg.CNA, reg.CNA_DMA_CON2): 0x0ffffffd,
                (reg.CNA, reg.CNA_WEIGHT_SIZE0): full_weight,
                (reg.CNA, reg.CNA_WEIGHT_SIZE1): full_weight >> 8,
                (reg.CNA, reg.CNA_WEIGHT_SIZE2): (s["kw"] << 24) | (s["kh"] << 16) | s["out_c"],
                (reg.CNA, reg.CNA_CONV_CON2): 0x20,  # FEATURE_GRAINS=2 per GEM2
                (reg.CORE, reg.CORE_DATAOUT_SIZE_1): s["out_c"] - 1,
                (reg.DPU, reg.DATA_CUBE_CHANNEL): ((s["out_c"] - 1) << 16) | (s["out_c"] - 1),
                (reg.DPU, reg.WDMA_SIZE_0): s["out_c"] - 1,
            })
        # All 3 tasks are identical (rknn uses shared cbuf + 3 PC-chain submissions)
        rows = [row_setup(), row_setup(), row_setup()]
        if tuple(len(row) for row in rows) != layout["amounts"]:
            raise RuntimeError(f"{s['name']} 3-setup row amounts changed")
        return rows
...
def exact_byk_legacy_layout_check(s):
    if s["name"] == C128_H1_OC24_3SETUP_SHAPE:
        # 3-setup chain: 3 IDENTICAL setup rows of 108q (per live c128_h1_oc24 GEM1
        # shows all 3 tasks pointing to the same regcmd_addr with same 108q body).
        return {
            "roles": ("setup_body", "setup_body", "setup_body"),
            "amounts": (108, 108, 108),
            "masks": (0x0d, 0x0d, 0x0d),
            "pc_amounts": (0, 0, 0),
            "offsets": (0, 112, 224),
        }
    ...
    elif s["name"] == C480_H14_OC16_EXACT11_SHAPE:
        # 11-task no-khalf layout: 1 setup + 5 k_tiles (104q) + 5 aux (26q) interleaved
        # Roles/amounts match EXACT11_BYK_AMOUNTS (the materializer in _exact11_task_regs
        # emits 1 setup + 5 k_tiles + 5 aux with no k_half rows).
        return no_khalf_5tile_layout(((0, 4), (4, 3), (7, 3), (10, 3), (13, 3)))
    return layout


def no_khalf_5tile_layout(splits):
    """Generic 11-task no-khalf layout: 1 setup + 5 k_tiles + 5 aux (no k_half rows)."""
    roles = ["setup_body"]
    aux_idx = 0
    for idx, _split in enumerate(splits):
        roles.append(f"k_tile_body{idx}")
        if idx + 1 < len(splits):
            roles.append(f"aux{aux_idx}")
            aux_idx += 1
    roles.append(f"aux{aux_idx}")
    roles = ["setup_body"]
    aux_idx = 0
...
def _c128_h2_oc256_exact11_task_regs(s, in_dma, wt_dma, out_dma):
    if s["name"] != C128_H2_OC256_EXACT11_SHAPE:
        raise ValueError("c128_h2_oc256 exact11 rows are scoped only to the c128_h2_oc256 shape")
    half = _align_up(s["out_c"] // 2, 16)
    ...
    def row(family, oc_start, oc_count, weight_size0):
        ...
        return patch_regs(regs, {
            (reg.CNA, reg.CNA_DATA_SIZE1): 0x003f0080,
            (reg.CNA, reg.CNA_CBUF_CON0): 0x01b,
            (reg.CNA, reg.CNA_CVT_CON0): 0x000b,
            (reg.CNA, reg.CNA_DMA_CON2): 0x0ffffffc,
            (reg.CNA, reg.CNA_WEIGHT_SIZE0): weight_size0,
            (reg.CNA, reg.CNA_WEIGHT_SIZE1): weight_size0 >> 8,
    ...
    rows = [
        row("setup", 0, s["out_c"], full_weight),
        row("k_half", 0, half, half * s["in_c"] * FP16_BYTES), _exact11_aux_regs(s, out_dma, aux_dma),
        row("k_half", half, s["out_c"] - half, (s["out_c"] - half) * s["in_c"] * FP16_BYTES), _exact11_aux_regs(s, out_dma, aux_dma),
    ]
    splits = exact_byk_splits(s)
    last_oc_start = splits[-1][0]
    for oc_start, oc_count in splits:
        rows.append(row("k_tile", oc_start, oc_count, oc_count * s["in_c"] * FP16_BYTES))
        if oc_start != last_oc_start:
            rows.append(_exact11_aux_regs(s, out_dma, aux_dma))
    rows.append(_exact11_aux_regs(s, out_dma, aux_dma))
    if tuple(len(row_regs) for row_regs in rows) != EXACT11_BYK_AMOUNTS:
        raise RuntimeError("c128_h2_oc256 exact11 row amounts changed")
    return rows
