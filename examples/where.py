...
def mem_allocate(fd, size, flags=0):
  mem = rknpu_mem_create(flags=flags | RKNPU_MEM_NON_CACHEABLE, size=size)
  ioctl(fd, DRM_IOCTL_RKNPU_MEM_CREATE, mem)
  mem_map = rknpu_mem_map(handle=mem.handle)
  ioctl(fd, DRM_IOCTL_RKNPU_MEM_MAP, mem_map)
  buf = mmap.mmap(fd, mem.size, mmap.MAP_SHARED, mmap.PROT_READ | mmap.PROT_WRITE, offset=mem_map.offset)
  return buf, mem
...
def run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, regs):
  ...
  reset_npu(fd)
  ret = submit(fd, task_mem.obj_addr)
...
def main():
  ...
  try:
    ...
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, generic_regs(n, mem[0].dma_addr, mem[1].dma_addr, mem[2].dma_addr, ew_cfg(4)))
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, cmplt_regs(n, mem[2].dma_addr, mem[5].dma_addr))
    ...
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, generic_regs(n, mem[3].dma_addr, mem[5].dma_addr, mem[8].dma_addr, ew_cfg(0, op_type=1, op_cvt_bypass=True)))
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, generic_regs(n, mem[3].dma_addr, mem[5].dma_addr, mem[2].dma_addr, ew_cfg(0, op_type=1, op_cvt_bypass=True)))
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, generic_regs(n, mem[6].dma_addr, mem[5].dma_addr, mem[7].dma_addr, ew_cfg(4)))
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, generic_regs(n, mem[4].dma_addr, mem[7].dma_addr, mem[8].dma_addr, ew_cfg(0, op_type=1, op_cvt_bypass=True)))
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, generic_regs(n, mem[4].dma_addr, mem[7].dma_addr, mem[6].dma_addr, ew_cfg(0, op_type=1, op_cvt_bypass=True)))
    run_regs(fd, tasks, task_mem, regcmd, regcmd_mem, generic_regs(n, mem[2].dma_addr, mem[6].dma_addr, mem[7].dma_addr, ew_cfg(2)))
