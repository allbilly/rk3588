...
TARGETS = {
    ...
    "c576_h19_pw_by_y": {
        ...
        "emit": None,
        "dump": "/home/orangepi/npu/ops_rknn/dump/prefix_c576_h19_oc12_s1pvalid_keep1_gem1/dump_gem1.txt",
        "note": "BODY FIELDS VALIDATED: GEM2 dump at /home/orangepi/npu/ops_rknn/dump/prefix_c576_h19_oc12_s1pvalid_keep1_gem2/dump_gem2.txt confirms 12-task structure (1 setup(108q) + 1 k_half(108q, 4q prelude) + 5 k_tile(104q) + 5 aux(26q)) with k_splits 4+3+2+1+2=12. ACTUAL body field constants: CBUF0=0x1b (NOT the c256_h2_oc64 guess 0xb1), DATA_SIZE1=0x3f0240 (NOT 0x8f0240), DMA_CON2=0x011d (NOT 0x0ffffffc), CVT_CON0=0x000b. CONV2 family bits: setup=0x80, k_half=0x40 (with CBUF0=0x201b weight_reuse), k_tile=0x60. WEIGHT_SIZE0=0x3600 (12*576*2=13824, full 12-OC weight for all rows), WEIGHT_SIZE1=0x480, WEIGHT_SIZE2=0x1010100c. examples/conv_c576_h19_oc12_no_submit.py task_structure check still passes with the old guessed values. examples/conv.py c576_h19_oc12_exact12 materializer (12 tasks with subcores=(0,1)x3) submits end-to-end without crashing, but per-row dst/feature offsets from the dump are 0x1300, 0, -0x420, 0, 0x850, 0xf70 for the 6 k_tile rows and were not fully patched into the per-row y_start calculation; max_diff=152.1078 with the simple y_start=0 for all rows. Follow-up needs per-row y_start patches using a stride of 19*8*2=304 bytes per row (the 0x1300 forward offset is consistent with y_start=16, the 0x850 with y_start=7, and the 0xf70 with y_start=13, but -0x420 backward is not an integer multiple of 304). Post simple_add PASS, pre simple_add PASS. Fresh GEM2 capture was the breakthrough that broke the educated-guess barrier; promotion now needs a per-row offset decode that maps the dump addresses to y_start values.",
    },
    "c128_h80_pw_by_y": {
