...
| 2026-06-02 16:54 | 115 | 102 | 0 | 0 | 0 | offline writer-tail equivalence passed for legacy 11-task exact BY_K layout. `simple_add.py` still fails before submit at BO mmap; sysfs shows runtime suspended/usage 0. |
| 2026-06-02 18:19 | **122** | **95** | **0** | **0** | **0** | h14 pointwise exact11 BY_K batch promoted (oc112/oc96×3/oc512 + 4 `_s1_pvalid` aliases); sweep `/tmp/opencode/conv_py_217_sweep_20260602_181917_summary.txt` PASS=122/FENCED=95, pre/post health rc=0. |
| 2026-06-02 20:51 | 132 | 85 | 0 | 0 | 0 | 10 more promotions (c192_h28_oc96, c256_h2_oc24, c256_h3_oc24, c40_h40_oc320, etc.). Sweep `/tmp/opencode/conv_py_217_sweep_20260602_205149_summary.txt` ran with `--skip-health`; separate `simple_add.py` checks passed around probes. |
| 2026-06-02 21:10 | 133 | 84 | 0 | 0 | 0 | c256_h3_oc24 promoted via shape-scoped exact11 compact-weight PC-chain replay; sweep `/tmp/opencode/conv_py_217_sweep_20260602_211043_summary.txt` pre/post health rc=0. |
| 2026-06-02 22:24 | 134 | 83 | 0 | 0 | 0 | c40_h40_oc320 promoted via one-task setup108 compact-weight replay; sweep `/tmp/opencode/conv_py_217_sweep_20260602_222614_summary.txt` pre/post health rc=0. |
| 2026-06-02 22:30 | 134 | 83 | 0 | 0 | 0 | c256_h2 oc64/oc546 added to `CRASH_FENCED_SHAPES` after speculative local submit rebooted board; direct runs now exit before DRM/submit. sweep_20260602_223030 PASS=134/FENCED=83. |
| 2026-06-03 00:01 | **136** | **81** | **0** | **0** | **0** | c512_h7 pointwise promoted (both prefixed and unprefixed aliases); sweep `/tmp/opencode/conv_py_217_sweep_20260603_000137_summary.txt` pre/post health rc=0. |
| 2026-06-03 09:49 | **136** | **81** | **0** | **0** | **0** | **CURRENT** — fresh `python3 sweep_217.py --output-dir /tmp/opencode` re-verified PASS=136/FENCED=81; `python3 examples/simple_add.py` PASS both before and after; `uptime 11:42` (no reboot). Continued safe `KEEP_TASKS=1 PREFIX_MODE=linear DUMP_GEM=1/2` prefix replay on depthwise BY_YK (c96_h112, c144_h56) and pointwise BY_K (c512_h7_oc1024, c832_h7_oc48) and crash-fenced c256_h2_oc64 — no new promotions without proven no-submit parity. |
| **Target** | **217** | **0** | **0** | **0** | **0** | end state |

**Distance to goal:** 81 fenced → 0 fenced. Largest remaining blocks: ~44 `BY_K/k_tile` (mixed subfamilies: narrow pointwise like c832_h7_oc48 has hidden buffer/layout parity issues that still block local promotion), ~15 `BY_YK/k_half,k_tile,setup` (depthwise c96/c128/c144/c256 + pointwise c256_h28 with 14-row closure), 1 `NONE/setup` (c16_h80_oc128 3x3 needs custom 4-task writer), 2 crash-fenced narrow pointwise (c256_h2 oc64/oc546, keep fenced until exact local regcmd parity + safe prefix numeric PASS). Attack A 1x1 sub-family path is now proven via c64_h1_oc128 and ready to scale.
