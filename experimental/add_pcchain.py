...
def add_segment(input_dma, weight_dma, output_dma, next_dma, segment_amount):
    body = [
...
def main():
    ...
    try:
        ...
        input_map, input_mc = mem_allocate(fd, TASKS * 4096, RKNPU_MEM_NON_CACHEABLE)
        weight_map, weight_mc = mem_allocate(fd, TASKS * 4096, RKNPU_MEM_NON_CACHEABLE)
        output_map, output_mc = mem_allocate(fd, TASKS * 4096, RKNPU_MEM_NON_CACHEABLE)
        ...
        for task_idx, (aval, bval) in enumerate(VALUES):
            base_input = task_idx * 2048
            base_output = task_idx * 2048
            for i in range(8):
                inputs[base_input + i] = aval
                weights[base_input + i] = bval
                outputs[base_output + i] = 0
        ...
        got = [[outputs[task_idx * 2048 + i] for i in range(8)] for task_idx in range(TASKS)]
        expected = [[a + b] * 8 for a, b in VALUES]
        ok = ret == 0 and got == expected
        print(f"submit ret={ret} mode={args.mode} flags=0x{args.flags:x}")
        print("got", got)
