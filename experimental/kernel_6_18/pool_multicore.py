...
def submit_multicore(fd, tasks, task_count, core_ranges, core_mask, in_bos, out_bos):
  ret = 0
  for task_idx in range(task_count):
    single_task = (pool.struct_rknpu_task * 1)(tasks[task_idx])
    ret = pool.submit(fd, single_task, 1, in_bos=in_bos, out_bos=out_bos)
  return ret, SimpleNamespace(core_mask=core_mask, task_number=task_count)
...
def main():
  ...
  try:
    ...
    for bo in (regcmd_mc, input_mc, output_mc):
      pool.rt.fini_bo(fd, bo)
    ret, submit_struct = submit_multicore(
      fd, tasks, len(ops), core_ranges, args.core_mask,
      in_bos=[regcmd_mc, input_mc],
      out_bos=[output_mc])
    pool.rt.prep_bo(fd, output_mc)
