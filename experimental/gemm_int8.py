...
def pack_weight_tile_16x32(m, n, k, b_matrix, wt_pack, align_in, align_out):
    wt = np.zeros((align_out, align_in), dtype=np.int8)
    wt[:n, :k] = b_matrix.T[:n, :k]
    wt_pack[:] = wt.reshape(align_out // 32, 2, 16, align_in // 32, 32).transpose(0, 3, 1, 2, 4).ravel()
...
def _gemm_layout(m, n, k):
    ...
    return aligned_k, align_out, k, False
...
def _rk_cbuf_data_entries(align_in):
    return _ceil_div(align_in, INT8_CBUF_ENTRY_VALUES)

def _rk_line_stride(m, n, k, eff_k):
    return max(1, _align_up(k, MIN_CHANNEL_TILE) // 16)
...
def _rk_notch_value(m, k, n, align_out, eff_k):
    ...
    return C2_INPUT * notch_blocks - 1
...
def make_gemm_regs(m, n, k, in_dma, wt_dma, out_dma):
    ...
    # GROUP_LINE_OFF is disabled only for the vendor KN/C2 schedules. Plain
    # aligned squares like 96x96x96 still need GROUP_LINE_OFF set.
    ...
    npu_regs = [
        ...
        E(reg.TARGET_CNA,  reg.CNA_WEIGHT_SIZE0, align_in * align_out),
        E(reg.TARGET_CNA,  reg.CNA_WEIGHT_SIZE1, align_in),
        ...
        E(reg.TARGET_CORE, reg.CORE_MISC_CFG,       1),
        ...
        E(reg.TARGET_DPU,  reg.DATA_FORMAT,       4 << 29),
        ...
        E(reg.TARGET_DPU,  reg.BS_OW_CFG,         0x000007fc),
        ...
        E(reg.TARGET_DPU,  reg.SURFACE_ADD,       0x00000080),
...
def run_gemm(m, n, k, a_matrix, b_matrix):
    ...
    a_matrix = np.asarray(a_matrix, dtype=np.int8, order="C").reshape(m, k)
    b_matrix = np.asarray(b_matrix, dtype=np.int8, order="C").reshape(k, n)
    in_pack = np.zeros(align_in * m, dtype=np.int8)
    wt_pack = np.zeros(align_in * align_out, dtype=np.int8)
    ...
    ct_inputs = (ctypes.c_int8 * len(in_pack)).from_buffer(input_map)
    ct_weights = (ctypes.c_int8 * len(wt_pack)).from_buffer(weight_map)
...
if __name__ == "__main__":
    test_cases = [
        ...
        np.array([[5, 6]], dtype=np.int8)),
    ]
    np.random.seed(42)
    square_sizes = (8, 9) + tuple(1 << exp for exp in (6, 8))
    for size in square_sizes:
        m = n = k = size
        a = np.random.randint(-32, 32, size=(m, k), dtype=np.int8)
        b = np.random.randint(-32, 32, size=(k, n), dtype=np.int8)
