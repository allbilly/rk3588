...
class CSCSim:
    ...
    def trace(self, wpack, input_nchw, out_c, in_c, kh, kw, groups=1,
        ...
        if is_dw:
            ...
            if dtype in ("int8", "fp16_dw_block"):
...
def _stream_ic_stride(dtype, in_c_eff, ic_atom):
    if dtype in ("fp16", "fp16_dw_block"):
        return ic_atom
...
# ---------------------------------------------------------------------------
# Fast packing experiments for simple_conv_fp16.py cleanup
# ---------------------------------------------------------------------------

def pack_weights_fp16_kx_major(weight_nchw, out_c, in_c, kh, kw, ic_atom=32, groups=1):
    if _is_depthwise(in_c, out_c, groups):
        raise ValueError("kx-major experiment is for non-depthwise weights")
    aligned_in_c = _align_up(in_c, ic_atom)
    pack_out_c = _compute_pack_out_c(out_c, in_c, kh, kw, groups)
    padded = np.zeros((pack_out_c, aligned_in_c, kh, kw), dtype=np.float16)
    padded[:out_c, :in_c] = weight_nchw[:out_c, :in_c]
    blocks = []
    for oc in range(0, pack_out_c, 16):
        block = padded[oc:oc + 16]
        block_oc = block.shape[0]
    ...
    return WeightPack(np.concatenate(blocks), pack_out_c, aligned_in_c, ic_atom,
                      oc_atom=16, kh=kh, kw=kw, flip_kspatial=True, dtype="fp16")

def pack_weights_fp16_depthwise_block_major(weight_nchw, out_c, in_c, kh, kw, ic_atom=32):
    aligned_in_c = _align_up(in_c, ic_atom)
    packed = np.zeros((aligned_in_c // ic_atom, kh, kw, ic_atom), dtype=np.float16)
    for c in range(min(out_c, in_c)):
        packed[c // ic_atom, :, :, c % ic_atom] = weight_nchw[c, 0]
    return WeightPack(packed.ravel(), out_c, aligned_in_c, ic_atom,
                      oc_atom=1, kh=kh, kw=kw, flip_kspatial=False,
                      dtype="fp16_dw_block")

def _roundtrip_feature_input_fp16(input_nchw, width_stride, aligned_c, c2):
    packed = np.zeros((_ceil_div(aligned_c, c2), input_nchw.shape[1], width_stride, c2), dtype=np.float16)
    for channel in range(input_nchw.shape[0]):
        packed[channel // c2, :, :input_nchw.shape[2], channel % c2] = input_nchw[channel]
    out = np.zeros_like(input_nchw)
    for channel in range(input_nchw.shape[0]):
        out[channel] = packed[channel // c2, :, :input_nchw.shape[2], channel % c2]
    return out

def _fp16_im2col(input_nchw, weight_nchw, out_c, in_c, kh, kw):
    _, _, in_h, in_w = input_nchw.shape
    out_h = in_h - kh + 1
    out_w = in_w - kw + 1
    flat_c = in_c * kh * kw
    im = np.zeros((1, flat_c, out_h, out_w), dtype=np.float16)
    wt = np.zeros((out_c, flat_c, 1, 1), dtype=np.float16)
    off = 0
    for ic in range(in_c):
        for ky in range(kh):
            for kx in range(kw):
                im[0, off] = input_nchw[0, ic, ky:ky + out_h, kx:kx + out_w]
                wt[:, off, 0, 0] = weight_nchw[:, ic, ky, kx]
                off += 1
    return im, wt

def _run_pack_experiment(label, fn):
    try:
        fn()
        return label, "PASS", ""
...
def _experiment_force_ncxhwcx_small_input():
    rng = np.random.default_rng(6100)
    for c in (1, 2, 3, 4):
        x = rng.uniform(-1, 1, (c, 5, 7)).astype(np.float16)
        for c2 in (2, 4, 8, 16):
            aligned_c = _align_up(c, max(c2, 8))
            y = _roundtrip_feature_input_fp16(x, 8, aligned_c, c2)
            assert np.array_equal(x, y), (c, c2)

def _experiment_fp16_kx_major_weights():
    rng = np.random.default_rng(6200)
    cases = [(3, 6, 2, 3), (16, 17, 3, 2), (32, 64, 3, 3)]
    for in_c, out_c, kh, kw in cases:
        x = rng.uniform(-1, 1, (1, in_c, kh + 2, kw + 2)).astype(np.float16)
        w = rng.uniform(-1, 1, (out_c, in_c, kh, kw)).astype(np.float16)
        wpack = pack_weights_fp16_kx_major(w, out_c, in_c, kh, kw)
        _, _, ok, md = CSCSim().verify(wpack, x, w, out_c, in_c, kh, kw, atol=0.2)
        assert ok, (in_c, out_c, kh, kw, md)

def _experiment_fp16_depthwise_block_major():
    rng = np.random.default_rng(6300)
    for c in (1, 17, 32, 33, 64, 65):
        kh, kw = 3, 2
        x = rng.uniform(-1, 1, (1, c, kh + 2, kw + 2)).astype(np.float16)
        w = rng.uniform(-1, 1, (c, 1, kh, kw)).astype(np.float16)
        wpack = pack_weights_fp16_depthwise_block_major(w, c, c, kh, kw)
        _, _, ok, md = CSCSim().verify(wpack, x, w, c, c, kh, kw, groups=c, atol=0.2)
        assert ok, (c, md)

def _experiment_fp16_im2col_small_spatial():
    rng = np.random.default_rng(6400)
    for in_c, out_c, kh, kw in ((1, 6, 3, 5), (3, 6, 3, 3), (4, 8, 2, 3)):
        x = rng.uniform(-1, 1, (1, in_c, kh + 3, kw + 3)).astype(np.float16)
        w = rng.uniform(-1, 1, (out_c, in_c, kh, kw)).astype(np.float16)
        im, wt = _fp16_im2col(x, w, out_c, in_c, kh, kw)
        direct = naive_conv(x, w, out_c, in_c, kh, kw)[0]
        one_by_one = naive_conv(im, wt, out_c, im.shape[1], 1, 1)[0]
        assert np.allclose(direct, one_by_one, atol=0.2), (in_c, out_c, kh, kw)

def _experiment_group_decompose_matches_dense():
    rng = np.random.default_rng(6500)
    for in_c, out_c, groups, kh, kw in ((4, 8, 2, 3, 3), (15, 35, 5, 3, 3), (6, 18, 3, 1, 1)):
        x = rng.uniform(-1, 1, (1, in_c, kh + 2, kw + 2)).astype(np.float16)
        wg = rng.uniform(-1, 1, (out_c, in_c // groups, kh, kw)).astype(np.float16)
        dense = naive_conv(x, expand_grouped_weights(wg, in_c, out_c, kh, kw, groups), out_c, in_c, kh, kw)[0]
        parts = np.zeros_like(dense)
        out_per_group = out_c // groups
        in_per_group = in_c // groups
        for g in range(groups):
            xs = x[:, g * in_per_group:(g + 1) * in_per_group]
            ws = wg[g * out_per_group:(g + 1) * out_per_group]
            parts[g * out_per_group:(g + 1) * out_per_group] = naive_conv(xs, ws, out_per_group, in_per_group, kh, kw)[0]
        assert np.allclose(dense, parts, atol=0.2), (in_c, out_c, groups)

def _experiment_ic_chunk_sum_matches_direct():
    rng = np.random.default_rng(6600)
    for in_c, out_c in ((96, 24), (144, 32), (256, 32)):
        x = rng.uniform(-1, 1, (1, in_c, 3, 3)).astype(np.float16)
        w = rng.uniform(-1, 1, (out_c, in_c, 1, 1)).astype(np.float16)
        direct = naive_conv(x, w, out_c, in_c, 1, 1)[0]
        acc = np.zeros_like(direct)
        for start in range(0, in_c, 64):
            end = min(start + 64, in_c)
            acc += naive_conv(x[:, start:end], w[:, start:end], out_c, end - start, 1, 1)[0]
        assert np.allclose(direct, acc, atol=0.2), (in_c, out_c)

def run_packing_experiments():
    experiments = [
        ("force_ncxhwcx_small_input", _experiment_force_ncxhwcx_small_input),
        ("fp16_kx_major_weights", _experiment_fp16_kx_major_weights),
        ("fp16_depthwise_block_major", _experiment_fp16_depthwise_block_major),
        ("fp16_im2col_small_spatial", _experiment_fp16_im2col_small_spatial),
        ("group_decompose_matches_dense", _experiment_group_decompose_matches_dense),
        ("ic_chunk_sum_matches_direct", _experiment_ic_chunk_sum_matches_direct),
    ]
    results = [_run_pack_experiment(label, fn) for label, fn in experiments]
    name_width = max(len(label) for label, _, _ in results)
    failed = 0
    print("packing experiments:")
    for label, status, detail in results:
        failed += status != "PASS"
        suffix = f"  {detail}" if detail else ""
        print(f"  {label:<{name_width}}  {status}{suffix}")
    print(f"\npacking experiments: {'ALL PASS' if failed == 0 else f'{failed} FAILURES'}")
    return failed == 0
