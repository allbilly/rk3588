RKNN runtime strings update:

- `ref/rknn/librknnrt.so` is the official RKNN runtime shared object; full strings are saved in `ref/rknn/librknnrt.strings.txt`, and the tiling-related subset is saved in `ref/rknn/librknnrt_tiling_strings.txt`.
- The binary has explicit RK3588/F2 support: `RKNPU F2`, `RKNNRegisterTask_f2`, `RKNNTargetF2`, and `RKNPUEmitterF2`.
- The key conv string is `min_weight_banks > 3 && m_Target == RKNNTarget::RKNPU_F2, do conv with ChannelTile`. That is strong evidence that the official RK3588 runtime uses ChannelTile/OC-family conv tiling when minimum weight-bank pressure exceeds 3 banks.
- The runtime also has direct CNA group diagnostics for feature, weight, and CSC group0/group1, plus a reuse table with `data reuse`, `weight reuse`, `mc_treat_by_y_tile`, `mc_treat_by_k_tile`, `mc_treat_by_1c_y_tile`, and `mc_treat_by_1c_k_tile`.
- Practical correction: OC/ChannelTile is not just a `conv_new_clean.py` hack. It appears to be part of Rockchip's own F2 compiler strategy. The cleanup target should keep the OC/ChannelTile semantic family and refactor duplicated loops, not remove it in favor of im2col-everything or unproven ONNC-only weight streaming.

Updated experiment direction after strings:

- Re-run the weight-overflow proof as an RKNN-style ChannelTile experiment: choose a direct conv with `min_weight_banks > 3`, split output channels/kernel groups explicitly, preserve direct spatial layout, and inspect/compare reuse bits and CNA feature/weight/CSC group programming.
- Do not interpret the earlier forced-bank sweep as proof that ONNC-style partial-weight streaming is enough. The small sweep proves some weight reuse exists, but the official runtime string says F2 switches to ChannelTile at the relevant threshold.
- Keep `dense_spatial_im2col_oc_h` as the safe fallback until direct RKNN-style ChannelTile is reproduced. If reproduced, it should replace im2col for dense spatial overflow without deleting the OC tiling family.

RKNN-style ChannelTile experiment result:

- Added `--mode channel-tile` to `experimental/prove_conv_hw_weight_stream.py`.
- The proxy threshold shape `b1_c32_h14_w14_oc128_wic32_k3x3_g1` passes with 16-wide direct channel tiles when each tile is submitted separately and writes into the final `nc1hwc2` output buffer: `max_diff=0.030413`.
- The same proxy fails when all channel tiles are PC-chained with different weight-buffer offsets: final buffer differs from the per-submit stitched result by `max_diff=91.437500`. This suggests our current PC-chain path does not safely handle different per-task weight addresses, or needs extra reuse/group programming.
- The real overflow shape `b1_c160_h14_w14_oc320_wic160_k3x3_g1` still fails even with 16-wide channel tiles before output placement: per-submit final equals CPU stitch, but both are wrong (`max_diff=309.106822` natural data banks, `240.133552` full-data mode).
- Conclusion: basic direct ChannelTile is partly proven for the small F2 threshold proxy, including final output offsets. It is not enough for the real dense overflow. RKNN's `ChannelTile` likely includes additional split/reuse/CNA group programming beyond simple OC slicing.

Experiment correction after RK3588 runs:

- Not all proof experiments failed. The calibrated weight-bank sweep passed: a direct spatial conv with full-weight estimate 3 banks still produced correct output with forced weight banks down to 2, then failed at 1. That proves RK3588 has some hardware weight streaming/reuse behavior, but it does not yet handle the real 160->320 3x3 overflow case with the current direct registers.
- IC-split is also partially proven. Four `128->24` pointwise partial convs were individually correct, and CPU summation of their raw output layout matched NumPy. The failure is specifically the DPU reduction path reading/reducing conv-output BOs, not IC-split itself and not standalone FP16 DPU add.
- The direct replacement of `pointwise_oc_h_layout_fallback` with direct/H failed on the first tested layout fallback shape, so that fallback is not just cosmetic.

Updated practical conclusion: keep the safe current fallback families for cleanup. Treat RKNN-style `rknn_channel_tile_h`, ONNC-style `hw_weight_tile_h`, and `ic_split_sum_h` as research paths: ChannelTile is now the best next dense-spatial experiment because the official F2 runtime advertises it; ONNC weight streaming still needs KG/bank-boundary work; IC-split needs a correct NPU-side reduction path for conv-output buffers.
...
``` bash
strings librknnrt.so
```

RKNPU Emitter Classes — multiple NPU gens:
RKNPUEmitter      (base)
RKNPUEmitterF2    ← RK3588 (this is us)
RKNPUEmitterT4
RKNPUEmitterV2
RKNPUEmitterLite
Explicit ChannelTile threshold for each NPU gen:
min_weight_banks > 3 && RKNPU_F2 → do conv with ChannelTile   # ← RK3588!
min_weight_banks > 7 && RKNPU_F3 → do conv with ChannelTile
min_weight_banks > 7 && RKNPU_W1 → do conv with ChannelTile
min_weight_banks > 8 && RKNPU_W2 → do conv with ChannelTile
This means the official RK runtime uses ChannelTile (what we call OC-tiling) when weights exceed 3 CBUF banks on RK3588 — exactly the threshold our code uses!
Multi-core tiling:
mc_treat_by_y_tile | mc_treat_by_k_tile 
...
CNA group programming (the runtime programs CNA directly, same as our Python):
CNA csc     group0/group1    ← CSC groups
CNA feature group0/group1    ← feature data groups  
CNA weight  group0/group1    ← weight groups (this is weight split-K!)
Split/reuse infrastructure:
Cannot find uniform split solution for size %d with limit %d and macC %d
Meet unsupported hybrid split type / split
Invalid reuse strategy!
failed to update data and weight reuse!
X tile buffer overflow! ... Fatal Error input W too large
The official RK runtime's strategy is: ChannelTile (OC-tile) for weight overflow, with data/weight reuse config, multi-core Y/K splitting, and CNA group programming — matching our conv_new_clean.py approach much more than ONNC's HW weight stream. The threshold min_weight_banks > 3 for F2 confirms we're on the right track.